...
ALTER TABLE access_groups
    ADD COLUMN max_stream_bitrate_kbps integer NOT NULL DEFAULT 0
        CHECK (max_stream_bitrate_kbps >= 0);
...
BEGIN
 ...
 NEW.max_streams,NEW.max_transcodes,NEW.max_stream_bitrate_kbps,NEW.allowed_permissions,NEW.requests_allowed,NEW.is_default,NEW.created_at,NEW.updated_at)
 ...
 OLD.max_streams,OLD.max_transcodes,OLD.max_stream_bitrate_kbps,OLD.allowed_permissions,OLD.requests_allowed,OLD.is_default,OLD.created_at,OLD.updated_at) THEN
  NEW.configuration_revision := nextval('access_group_configuration_revision_seq');
 ELSE
...
ALTER TABLE users DROP COLUMN max_stream_bitrate_kbps;
ALTER TABLE access_groups DROP COLUMN max_stream_bitrate_kbps;
