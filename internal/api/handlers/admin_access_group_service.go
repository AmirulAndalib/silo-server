...
func (h *AccessGroupHandler) CreateAdminAccessGroup(ctx context.Context, in access.CreateGroupInput) (*access.Group, error) {
	...
	update := access.UpdateGroupInput{Name: &in.Name, LibraryIDs: &in.LibraryIDs, MaxPlaybackQuality: &in.MaxPlaybackQuality, MaxStreams: &in.MaxStreams, MaxTranscodes: &in.MaxTranscodes, MaxStreamBitrateKbps: &in.MaxStreamBitrateKbps, AllowedPermissions: &in.AllowedPermissions}
...
func normalizeAdminGroupInput(in *access.UpdateGroupInput) error {
	...
	if in.MaxStreams != nil && *in.MaxStreams < 0 || in.MaxTranscodes != nil && *in.MaxTranscodes < 0 || in.MaxStreamBitrateKbps != nil && *in.MaxStreamBitrateKbps < 0 {
		return ErrInvalidAccessGroup
	}
