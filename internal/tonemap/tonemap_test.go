...
func TestResolveSourceRejectsDolbyOnlyAndPreflightsAmbiguousMetadata(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "legacy row lacks presence flags", mutate: func(source *SourceMetadata) { source.DVConfigPresent = false; source.DVBLCompatIDPresent = false }, want: SourcePQ, wantPreflight: true},
