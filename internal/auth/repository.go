...
const allColumns = `id, email, username, password_hash, local_password_login_enabled, role, permissions, enabled,
	...
	max_streams, max_transcodes, max_stream_bitrate_kbps, transcode_allowed, audio_transcode_allowed, max_profiles, download_allowed,
...
func createUser(ctx context.Context, db interface {
	...
	cols := []string{
		...
		"library_ids", "max_playback_quality", "max_streams", "max_transcodes", "max_stream_bitrate_kbps",
...
func updateUser(ctx context.Context, db interface {
	...
	columns := []userUpdateColumn{
		...
		{column: "max_stream_bitrate_kbps", set: input.MaxStreamBitrateKbps.Set, value: input.MaxStreamBitrateKbps.Value},
