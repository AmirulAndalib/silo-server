...
`max_stream_bitrate_kbps` is a nullable account override. `null` inherits the
...
use one limit for local and remote playback. A policy edit changes new playback
sessions, not streams already playing.
